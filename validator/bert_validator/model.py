...
class MyModel(nn.Module):
    def __init__(self):
        ...
        # 添加dropout层（使用args.dropout_rate）
        ...
        # 自动获取bert的隐藏层大小，分类器层（二分类任务）
        self.linear = nn.Linear(self.bert.config.hidden_size, self.args.class_num)
    ...
    def forward(self, x):
        ...
        # 获取[CLS]标记的表示:
        pooled_output = hidden_out.pooler_output

        # 应用dropout
        # pooled_output = self.dropout(pooled_output)

        pred = self.linear(pooled_output)
